/**
 * tail — Darwin built-in tool (V3_ROADMAP P1). No npm deps, no shell.
 * Walks `cwd` recursively, returns last N lines of each matching file.
 * Result: { files: [{ file, lines: string[] }] }. Errors skipped silently.
 * n=0 → unlimited. Empty file → { file, lines: [] }. Symlinks followed.
 * Walker + include-glob compiler are duplicated from head.js / wc.js to
 * keep tail a single-responsibility leaf tool (no shared module).
 */
...
/** Compile a glob include pattern into a posix-path tester. Mirrors head.js. */
...
/** Last N lines of one file. null on skip, [] for empty. GNU tail semantics:
 *  trailing '' from final \n is not a line — drop it. slice(-n) preserves
 *  file order (first array element = earliest of the kept tail window). */
function tailLines(absPath, n) {
  let st;
  try {
    st = statSync(absPath);
  } catch {
  ...
  // trailing '' from final \n is not a line in GNU tail — drop it
  if (all[all.length - 1] === '') {
    all.pop();
  }
...
export const tail = {
  name: 'tail',
  description:
    'Return the last N lines of each file under cwd. ' +
  ...
  async execute({ cwd, include, n } = {}) {
    ...
    for (const full of files) {
      ...
      const lines = tailLines(full, limit);
      if (lines === null) {
        continue;
      }
