...
describe('PHASES constant', () => {
  test('exports exactly 5 phases', () => {
    assert.equal(PHASES_ORDER.length, 5);
  });

  test('phase names are INIT / CONFIG / CONTAINER / REGISTRY / READY in order', () => {
...
describe('PHASE_EVENTS mapping', () => {
  ...
  test('PHASE_EVENTS points at the right EVENTS.* constants', () => {
    // INIT/CONFIG/CONTAINER/REGISTRY share one bootstrap:start contract; READY → bootstrap:done
